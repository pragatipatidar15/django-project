...
def analyze(request,**kwargs):
    ...
    for eachEntry in data:
        if eachEntry.componentName == 'hw':
            heaightAndWeight=eachEntry.componentValue.split('/')
            if len(heaightAndWeight) >1 :
                feetToMetres=float(heaightAndWeight[0])*0.4536
                BMI=(float(heaightAndWeight[1]))/(feetToMetres*feetToMetres)
